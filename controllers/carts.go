...
func (app *Application) GetItemFromCart() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		err = app.userCollection.FindOne(ctx, bson.D{primitive.E{Key: "_id", Value: userst_id}}).Decode(&cartholder)
		if err != nil {
			log.Panic(err)
		...
		cursor, er := app.userCollection.Aggregate(ctx, mongo.Pipeline{filter_match, unwind, grouping})
		if er != nil {
			log.Panic(er)
